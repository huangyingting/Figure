...
export function LibraryControls({ figures }: { figures: FigureCardData[] }) {
  ...
  return <>
    <div className="mb-[22px] flex flex-col items-stretch justify-between gap-3 sm:flex-row sm:items-center">
      <label className="flex min-h-[46px] w-full items-center gap-[10px] rounded-[10px] border border-line bg-white px-[13px] sm:w-[min(430px,60%)]">
    ...
    {shown.length
      ? <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">{shown.map((figure, index) => <FigureCard key={figure.id} figure={figure} tone={["violet", "blue", "coral"][index % 3]} />)}</div>
