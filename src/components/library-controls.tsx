...
export function LibraryControls({ figures }: { figures: FigureCardData[] }) {
  ...
  return <>
    <div className="library-controls"><label><Search size={18} /><input value={query} onChange={(event) => setQuery(event.target.value)} aria-label="Search your figures" placeholder="Search your figures" /></label><CustomSelect compact label="Sort by" value={sort} onChange={setSort} options={[{ value: "newest", label: "Newest first" }, { value: "popular", label: "Most viewed" }]} /></div>
