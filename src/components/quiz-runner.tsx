...
export function QuizRunner({ figureId, title, parts }: { figureId: string; title: string; parts: AnnotatedPart[] }) {
  ...
  return <section className="quiz-runner"><header><div><p>VISUAL RECALL</p><h1>{title}</h1></div><span>{String(current + 1).padStart(2, "0")} <i>/</i> {String(questions.length).padStart(2, "0")}</span></header><div className="quiz-progress"><i style={{ width: `${current / questions.length * 100}%` }} /></div><div className="quiz-stage"><div className="quiz-image"><img src={`/api/figures/${figureId}/image`} alt="" /><span>Look closely</span></div><div className="quiz-question"><p>WHICH COMPONENT IS THIS?</p><h2>{question.part.description}</h2><div className="quiz-options">{question.options.map((option, index) => <button key={option.id} type="button" data-selected={selected === option.id} onClick={() => setSelected(option.id)}><span>{String.fromCharCode(65 + index)}</span>{option.name}{selected === option.id && (option.id === question.part.id ? <Check /> : <X />)}</button>)}</div><button className="quiz-next" disabled={!selected} onClick={() => void next()}>{current === questions.length - 1 ? "See results" : "Next question"}<ArrowRight size={17} /></button></div></div></section>;
}
