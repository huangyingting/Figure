...
export function QuizRunner({ figureId, title, parts, imageSrc, persist = true }: { figureId: string; title: string; parts: AnnotatedPart[]; imageSrc?: string; persist?: boolean }) {
  ...
  if (complete) return (
    <section className="grid min-h-[570px] place-items-center content-center rounded-[18px] border border-line bg-[radial-gradient(circle_at_50%_40%,rgb(255_201_77_/_28%),transparent_16rem),white] p-[50px] text-center">
      ...
      <p className="mt-[18px] mb-[3px] text-micro font-extrabold uppercase tracking-[0.13em] text-pine-dark">QUIZ COMPLETE</p>
      ...
      {saveFailed && <p className="mt-4 rounded-[9px] border border-[rgba(220,38,38,0.28)] bg-[rgba(220,38,38,0.06)] px-[13px] py-[9px] text-micro font-semibold text-[#dc2626]" role="alert">We couldn’t save this attempt. Your mastery history may not update.</p>}
