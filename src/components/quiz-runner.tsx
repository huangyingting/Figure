...
export function QuizRunner({ figureId, title, parts, imageSrc, persist = true }: { figureId: string; title: string; parts: AnnotatedPart[]; imageSrc?: string; persist?: boolean }) {
  ...
  if (complete) return <section className="quiz-complete"><span>{score === questions.length ? "✦" : "✓"}</span><p>QUIZ COMPLETE</p><h2>{score}/{questions.length}</h2><h3>{score === questions.length ? "Perfect visual recall." : score >= questions.length * .7 ? "You’re seeing the system." : "One more look will make it stick."}</h3><div><i style={{ width: `${score / questions.length * 100}%` }} /></div>{saveFailed && <p className="quiz-save-warning" role="alert">We couldn’t save this attempt. Your mastery history may not update.</p>}<button onClick={reset}><RotateCcw size={16} />Try again</button></section>;
  return <section className="quiz-runner"><header><div><p>VISUAL RECALL</p><h2>{title}</h2></div><span>{String(current + 1).padStart(2, "0")} <i>/</i> {String(questions.length).padStart(2, "0")}</span></header><div className="quiz-progress"><i style={{ width: `${current / questions.length * 100}%` }} /></div><div className="quiz-stage"><div className="quiz-image"><img src={imageSrc ?? `/api/figures/${figureId}/image`} alt="" /><span>Look closely</span></div><div className="quiz-question"><p>WHICH COMPONENT IS THIS?</p><h3>{question.part.description}</h3><div className="quiz-options">{question.options.map((option, index) => <button key={option.id} type="button" disabled={selected !== null} data-selected={selected === option.id} data-correct={selected !== null && option.id === question.part.id} onClick={() => setSelected(option.id)}><span>{String.fromCharCode(65 + index)}</span>{option.name}{selected !== null && option.id === question.part.id && <Check />}{selected === option.id && option.id !== question.part.id && <X />}</button>)}</div><button className="quiz-next" disabled={!selected} onClick={() => void next()}>{current === questions.length - 1 ? "See results" : "Next question"}<ArrowRight size={17} /></button></div></div></section>;
}
