...
export function MobileMenu({ account }: { account: MobileMenuAccount | null }) {
  ...
  return (
    <>
      ...
      {open && createPortal(
        <div className="fixed inset-0 z-[60] flex justify-end" role="dialog" aria-modal="true" aria-label={t("Navigation menu")}>
          <div
            ref={dialogRef}
          ...
          >
            ...
            {account && (
              <Button asChild size="md">
                <Link href="/studio" onClick={() => setOpen(false)}><Plus size={17} />{t("Create a figure")}</Link>
              </Button>
            )}
              ...
              {account ? (
                <>
                  ...
                  <Link className="flex items-center gap-[11px] text-inherit no-underline" href="/account" onClick={() => setOpen(false)}>
                    ...
                    <div><strong className="block text-meta">{account.name || t("Figure learner")}</strong><small className="text-micro text-muted">{account.email}</small></div>
                  </Link>
                  <Button
                    type="button"
                    variant="outline"
                ...
                <div className="grid gap-3 rounded-2xl border border-line bg-paper p-4">
                  ...
                  <LanguageSwitcher />
                  <p className="m-0 text-meta leading-[1.55] text-muted">{t("Sign in to create figures, build collections, and track mastery. New accounts start with 12 free credits.")}</p>
                  <Button asChild size="md">
                    <Link href="/signin" onClick={() => setOpen(false)}>{t("Sign in")}</Link>
                  </Button>
                </div>
              )}
            </div>
