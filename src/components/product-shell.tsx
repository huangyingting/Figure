      ...
      <aside className="fx-sidebar">
        ...
        <nav>
          ...
          {nav.map((item) => {
            ...
            return <Link key={item.href} href={item.href} data-active={active === item.href}><Icon size={18} />{item.label}</Link>;
          })}
        </nav>
        <div className="fx-sidebar-foot">
          {session?.user ? (
            <>
              <Link className="fx-credit-pill" href="/credits"><Coins size={16} /><span><strong>{session.user.credits}</strong> credits</span></Link>
