...
export function EditCollectionButton({ collectionId, name, description, color }: EditCollectionButtonProps) {
  ...
  return (
    <>
      ...
      {editing && (
        <div className="fixed inset-0 z-[100] grid place-items-center bg-[rgb(16_17_21_/_55%)] p-[25px] backdrop-blur-[7px]" role="presentation" onClick={() => setEditing(false)}>
          <section ref={dialogRef} className="relative w-[min(460px,100%)] rounded-[17px] bg-white p-[30px] shadow-[0_30px_90px_rgb(0_0_0_/_28%)]" role="dialog" aria-modal="true" aria-labelledby="edit-collection-title" onClick={(event) => event.stopPropagation()}>
            <button className="absolute right-4 top-4 grid h-[30px] w-[30px] cursor-pointer place-items-center rounded-lg border-0 bg-[#f2f1ed] [&_svg]:w-4" type="button" onClick={() => setEditing(false)} aria-label="Close"><X /></button>
            <p className="eyebrow m-0">EDIT COLLECTION</p>
            <h2 className="mb-[22px] mt-[7px] font-display text-[29px] tracking-[-0.05em]" id="edit-collection-title">Refine this collection.</h2>
