...
export function QuizFigureBrowser({
  ...
  const heading = browser.query
    ? `Results for “${browser.query}”`
    : "Recently added figures";

  return (
    <aside className="rounded-2xl border border-line-dark bg-paper p-4 shadow-card lg:sticky lg:top-4" aria-label="Choose a figure">
      <div className="mb-4">
        <p className="eyebrow m-0">CHOOSE A FIGURE</p>
        ...
        <p className="m-0 text-meta leading-[1.5] text-muted">
          {browser.query
            ? "Select a result to start a new visual recall quiz."
            : "Your newest accessible figures, ready for recall."}
        </p>
      </div>
      ...
      <form action="/quiz" className="mb-4 grid grid-cols-[1fr_auto] gap-2" role="search">
        ...
        <label className="flex min-h-[44px] min-w-0 items-center gap-2 rounded-[10px] border border-line bg-[#fbf7ec] px-3 focus-within:border-pine focus-within:shadow-[0_0_0_3px_rgb(28_107_82/8%)]">
          ...
          <input
            ...
            maxLength={120}
            aria-label="Search quiz figures"
            placeholder="Search title or topic"
            className="min-w-0 flex-1 border-0 bg-transparent text-ui outline-none placeholder:text-muted-2"
          />
        </label>
        <button type="submit" className="min-h-[44px] cursor-pointer rounded-[10px] border-0 bg-ink px-3 text-meta font-bold text-white hover:bg-pine" aria-label="Search figures">
          Search
        </button>
      </form>
      ...
      {browser.figures.length ? (
            ...
            return (
              ...
              >
                ...
                <span className="grid min-w-0 content-center gap-0.5">
                  <strong className="truncate text-ui text-ink group-hover:text-pine-dark">{figure.title}</strong>
                  <span className="truncate text-micro text-muted">{figure.subject}</span>
                  <span className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-0.5 text-[11px] text-muted-2">
                    <span className="inline-flex items-center gap-1"><Clock3 size={11} />{formatGeneratedDate(figure.createdAt)}</span>
        ...
        <EmptyState
          ...
          icon={browser.query ? "⌁" : "✦"}
          title={browser.query ? "No matching figures" : "No figures yet"}
          description={browser.query ? "Try a broader title or topic." : "Create a figure and it will appear here."}
          action={browser.query
            ? <Button asChild size="sm" variant="outline"><Link href={quizFigureHref({ figureId: selectedFigureId })}>Show recent</Link></Button>
            : <Button asChild size="sm"><Link href="/studio">Create a figure</Link></Button>}
        />
      )}

      {browser.query && (browser.hasPrevious || browser.hasNext) ? (
        <nav className="mt-4 flex items-center justify-between gap-2 border-t border-line pt-3" aria-label="Search result pages">
          {browser.hasPrevious ? (
            <Link className="text-meta font-bold text-pine-dark no-underline hover:underline" href={quizFigureHref({ figureId: selectedFigureId, query: browser.query, page: browser.page - 1 })}>← Previous</Link>
          ) : <span />}
          <span className="text-micro font-bold text-muted">Page {browser.page}</span>
          {browser.hasNext ? (
            <Link className="text-meta font-bold text-pine-dark no-underline hover:underline" href={quizFigureHref({ figureId: selectedFigureId, query: browser.query, page: browser.page + 1 })}>Next →</Link>
          ) : <span />}
        </nav>
      ) : null}
    </aside>
