...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!(await ownedFigure(id, session.user.id))) return NextResponse.json({ error: "Figure not found." }, { status: 404 });
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!annotation.success) return NextResponse.json({ error: "This revision can no longer be restored." }, { status: 409 });
