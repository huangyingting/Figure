...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!parsed.success) return NextResponse.json({ error: "Invalid figure." }, { status: 400 });
  ...
  if (!collection || !figure) return NextResponse.json({ error: "Collection or figure not found." }, { status: 404 });
