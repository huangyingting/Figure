...
export default async function CreditsPage() {
  ...
  return <ProductShell active="/credits"><Page>
    ...
    <section className="mb-7 grid grid-cols-1 overflow-hidden rounded-[18px] border border-line-dark bg-white shadow-[0_22px_60px_rgb(23_24_29_/_7%)] md:grid-cols-2">
      <div className="grid min-h-[230px] place-items-start content-center bg-[radial-gradient(circle_at_90%_10%,rgb(217_255_112_/_30%),transparent_16rem),var(--color-violet)] p-[34px] text-white">
        <p className="m-0 text-micro font-extrabold uppercase tracking-[0.13em] text-[#dbd8ff]">AVAILABLE BALANCE</p>
        <strong className="mt-1 block font-display text-[88px] leading-[0.9] tracking-[-0.09em]">{account.credits}</strong>
        <span className="text-lead font-bold">figure credits</span>
        <small className="mt-6 flex items-center gap-[7px] text-meta text-[#e4e1ff]"><Sparkles size={14} /> Each new account begins with 12 credits.</small>
      </div>
      <div className="grid content-center p-[34px]">
        <h2 className="mb-[17px] mt-0 font-display text-[25px] tracking-[-0.04em]">Everything is included.</h2>
        <ul className="m-0 grid list-none gap-3 p-0 [&_li]:relative [&_li]:pl-[22px] [&_li]:text-ui [&_li]:text-muted [&_li]:before:absolute [&_li]:before:left-0 [&_li]:before:font-extrabold [&_li]:before:text-green [&_li]:before:content-['✓']">
          <li>AI component planning</li>
          <li>High-resolution image generation</li>
          <li>Pixel-grounded annotations</li>
          <li>Permanent figure storage</li>
        </ul>
      </div>
    </section>
    <section className="overflow-hidden rounded-[14px] border border-line bg-white">
      <header className="flex items-center justify-between border-b border-line px-[21px] py-[18px]">
        <h2 className="m-0 font-display text-[20px]">Credit activity</h2>
        <span className="text-micro text-muted">{ledger.length} recent transactions</span>
      </header>
      {ledger.length ? ledger.map((item) => (
        <div key={item.id} className="grid grid-cols-[35px_1fr_auto] items-center gap-3 border-b border-[#efeeeb] px-[21px] py-[13px] sm:grid-cols-[35px_1fr_auto_58px] [&_svg]:w-[15px]">
          ...
          <div className="grid">
            <strong className="text-[10px]">{item.reason === "figure_generation" ? "Figure generated" : "Generation refunded"}</strong>
          ...
          <em className="hidden text-right text-micro not-italic text-muted sm:block">{item.balance} left</em>
        </div>
      )) : <div className="p-[35px] text-center text-[10px] text-muted">Your credit activity will appear after your first generation.</div>}
    </section>
  </Page></ProductShell>;
}
